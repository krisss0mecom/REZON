...
class Paper(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 7, "  Dense Associative Memory on S¹ — Gwóźdź (2025)  "
                        "| arXiv preprint | DOI: 10.5281/zenodo.18800042",
                  align="L", fill=True,
...
def build_pdf(fig_paths):
    ...
    pdf.cell(0, 5, "February 2025   |   DOI: 10.5281/zenodo.18800042", align="C",
             new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(6)
    ...
    pdf.body(
        ...
        "\u03c9_anc = 2\u03c0 \u00d7 200 Hz) for N \u2208 {32, 64, 128} oscillators. "
        "Natural frequency offsets \u03c9\u1d62 \u223c N(0, 0.01) rad/s are included in all simulations "
        "(mild intrinsic heterogeneity). "
        "Patterns \u03be\u03bc are binary {0,\u03c0}^N, generated by mapping i.i.d. Bernoulli bits to phases. "
        "For each (P, F, N) combination, 3 trials start from pattern \u03be\u03bd with 10% of bits "
        "toggled (0\u2194\u03c0) plus per-oscillator Gaussian jitter \u03b4\u03c6\u1d62 \u223c N(0, 0.05 rad), "
        "then evolve for 5000 warmup and 10000 recall steps. "
        "Recovery: b\u1d62 = 1[cos(\u03c6\u1d62) < 0], Hamming = \u03a3\u1d62 1[b\u1d62 \u2260 \u03be\u1d62^binary]. "
        "Scope: this protocol evaluates robustness to bit-flip + Gaussian jitter only; "
        "continuous-time phase drift or correlated noise is not evaluated here."
    )
    ...
    pdf.subsection("8.0 Role of the 200 Hz Anchor")
    pdf.body(
        "The anchor term a_anc\u00b7sin(\u03c9_anc\u00b7t \u2212 \u03c6\u1d62) serves three distinct functions:"
    )
    pdf.bullet("(i) Reference frame: breaks SO(1) rotational symmetry of E(\u03c6), "
               "providing the absolute phase reference required for binary decoding (b\u1d62 = 1[cos\u03c6\u1d62 < 0]).")
    pdf.bullet("(ii) Hardware enabler: injection-locking to a 200 Hz clock is the mechanism "
               "by which logic gates are realized in REZON.")
    pdf.bullet("(iii) Perturbation, not storage: the anchor does NOT store patterns or contribute to E(\u03c6). "
               "Its effect on recall is bounded (Proposition 1, a_anc = 0.08 << 1). "
               "Whether it actively assists or hinders convergence at large P is an open question.")
    ...
    pdf.multi_cell(0, 5.5,
        ...
        "ARM Cortex-A78AE). All code and data available at DOI: 10.5281/zenodo.18800042.")
    ...
    refs = [
        ...
        "Zenodo, doi:10.5281/zenodo.18800042 (2025).",
