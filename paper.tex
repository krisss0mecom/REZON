...
$\cos(\xi_i^\nu-\xi_i^\nu)=1$.
For $\mu\neq\nu$: random patterns give $|m_\mu|\ll N$ and
$F'(m_\mu)\ll F'(N)$ for superlinear $F$.
Hence $H_{ii} \approx F'(N) + O(P/\sqrt{N}) > 0$ for typical patterns,
confirming positive definiteness and local asymptotic stability.
\hfill$\square$
