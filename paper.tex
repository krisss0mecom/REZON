...
Recovery is declared if the Hamming distance to the target pattern equals zero.
Explicitly: we decode each oscillator via $b_i = \mathbf{1}[\cos\varphi_i < 0]
\in \{0,1\}$ (so $\varphi=0 \to b=0$, $\varphi=\pi \to b=1$),
and Hamming distance = $\sum_i \mathbf{1}[b_i \neq \xi_i^{\rm binary}]$.
This quantization matches the binary $\{0,\pi\}$ pattern encoding used throughout.
...
network size $N$. ($*$): $F=x^3$ is Euler-unstable at $N \ge 64$
because $\Delta t \cdot F'(N) = \Delta t \cdot N^2 \gg 1$
($= 4.1$ at $N=64$, $= 16.4$ at $N=128$); capacity there is
not meaningful. $F=\exp$ is numerically stable at all $N$ via
...
exceeding the classical Hopfield limit by a \textbf{7.2-fold} improvement.
...
serving a function \emph{analogous} to positional encoding in Transformers
(both break translational symmetry).
Crucially, however, the anchor enters the dynamics as a bounded non-autonomous
perturbation to the gradient flow (Proposition~1), not as an additive embedding;
the gradient-flow interpretation of the memory energy is unaffected by this
small forcing term ($a_{\rm anc}=0.08 \ll 1$).
...
Zenodo, \href{https://doi.org/10.5281/zenodo.18768137}{doi:10.5281/zenodo.18768137} (2025).
