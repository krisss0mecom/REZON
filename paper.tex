...
the exponential interaction $F = \exp$ achieves storage capacity
$\alpha^* = P^*/N = 1.0$ at both $N=32$ and $N=64$
(consistent with exponential capacity theory \cite{ramsauer2020})---a
factor of $\mathbf{7.2}{\times}$ above the classical Hopfield limit
($\alpha^* \approx 0.138$).
The polynomial interaction $F = x^3$ also reaches $\alpha^* = 1.0$ at $N=32$
but shows capacity roll-off at $N=64$ ($\alpha^*=0.094$), confirming that
only the exponential function maintains unit load as $N$ grows.
...
\begin{table}[h]
\centering
\caption{Storage capacity by interaction function $F$ and network size $N$.}
\label{tab:capacity}
\begin{tabular}{lrrrr}
\toprule
Interaction $F$ & \multicolumn{2}{c}{$N=32$} & \multicolumn{2}{c}{$N=64$} \\
 & $P^*$ & $\alpha^*$ & $P^*$ & $\alpha^*$ \\
\midrule
Linear ($F=x$)       & 1  & 0.031 & 1  & 0.016 \\
Quadratic ($F=x^2$)  & 9  & 0.281 & 12 & 0.188 \\
Cubic ($F=x^3$)      & 32 & 1.000 & 6  & 0.094 \\
Exponential ($F=e^x$)& 32 & \textbf{1.000} & 64 & \textbf{1.000} \\
\midrule
Classical Hopfield \cite{amit1985} & 4 & 0.138 & 9 & 0.141 \\
\bottomrule
\end{tabular}
\end{table}
...
for both $N=32$ ($\alpha^*=1.0$, 100\% trials) and $N=64$ ($\alpha^*=1.0$),
exceeding the classical Hopfield limit by a factor of $7.2\times$ in both cases.
$F=x^3$ also achieves $\alpha^*=1.0$ for $N=32$ (94.8\% trials) but shows
capacity roll-off at $N=64$ ($\alpha^*=0.094$), confirming that only
the exponential interaction maintains unit-load capacity as $N$ scales.
...
What is the finite-size scaling of $\alpha^*(N)$ for $N \gg 64$?
Can the 200\,Hz anchor be relaxed while maintaining capacity?
...
\begin{enumerate}
\item Storage capacity $\alpha^* = 1.0$ for $F=\exp$ at $N=32$ and $N=64$
  (and $F=x^3$ at $N=32$), a $7.2\times$ improvement over classical Hopfield.
  Only $F=\exp$ maintains unit load as $N$ scales.
